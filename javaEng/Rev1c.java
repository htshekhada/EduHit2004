...
public class Rev1c extends HttpServlet
{
...
//             String envf[] =gEnv.getAvailableFontFamilyNames();
       ...
       out.println("<form method=\"GET\" action=\"http://"+request.getServerName()+":8080/servlet/Revopt1\" target=\"bottom\">");
     ...
     out.println("<select  name=\"lno\" size=\"1\">");
      ...
      try  {
                 ...
                 rs = stmt.executeQuery("SELECT DISTINCT chap AS c1 FROM "+sub+std1+" where quefield='opt';");
                 int nRows = 0;
                 while(rs.next() ) {
