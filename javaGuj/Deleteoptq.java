...
public class Deleteoptq extends HttpServlet
{
...
*/      
     ...
     out.println("<form method=\"GET\" action=\"http://"+request.getServerName()+":8080/servlet/Deleteoptq1\">");
...
//     out.println("<input type=\"hidden\" name=\"subs\" value=\""+s2+"\">");
     ...
     try  {
                 ...
                 rs = stmt.executeQuery("SELECT min(queno) as c1 FROM "+s2+s1+" where quefield='opt' and queno >= "+q1+"-5  and chap="+lno+" ");
                 rs.next();
                 ccm = rs.getInt("c1");
                 
                 
                 rs = stmt.executeQuery("SELECT max(queno) as c1 FROM "+s2+s1+" where queno >= "+ccm+" and queno <= "+ccm+"+4 and quefield='opt' and chap="+lno+"");
                 rs.next();
                 cc = rs.getInt("c1");
                 ...
                 out.println("</strong>");
                 out.println("<br>");
                 rs = stmt.executeQuery("SELECT * FROM "+s2+s1+" where queno >= "+ccm+" and queno <= "+ccm+"+4 and quefield='opt' and chap="+lno+" order by queno ");
                 int nRows = 0;
                ...
                out.println("<th><font face=\"Lmg-Arun Condensed\" size=\"5\">BM8M HJFA</font>1</th>");
                out.println("<th><font face=\"Lmg-Arun Condensed\" size=\"5\">BM8M HJFA</font>2</th>");
     ...
     out.println("<center>");
     out.println("<form method=\"GET\" action=\"http://"+request.getServerName()+":8080/servlet/Deleteoptq\">");
